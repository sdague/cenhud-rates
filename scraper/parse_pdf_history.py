...
DELIVERY_CHARGES = {"standard": 0.13860, "on_peak": 0.14732, "off_peak": 0.12739}
...
def extract_rates(pdf_text):
    ...
    while i < len(lines):
        ...
        date_match = re.match(
            r"^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\.\s+\d{1,2},\s+202[56]",
            line,
        )
            ...
            if len(values) >= 3:
                ...
                rate_entry = {
                    ...
                    "standard": {
                        ...
                        "total_per_kwh": round(
                            standard_supply + DELIVERY_CHARGES["standard"], 5
                        ),
                    ...
                    "time_of_use": {
                        "on_peak": {
                            ...
                            "total_per_kwh": round(
                                onpeak_supply + DELIVERY_CHARGES["on_peak"], 5
                            ),
                        ...
                        "off_peak": {
                            ...
                            "total_per_kwh": round(
                                offpeak_supply + DELIVERY_CHARGES["off_peak"], 5
                            ),
                ...
                print(
                    f"Extracted rate for {effective_date}: Standard=${standard_supply}, On-Peak=${onpeak_supply}, Off-Peak=${offpeak_supply}"
                )
...
def save_historical_rates(
    rates, output_file="../custom_components/central_hudson/data/prices.json"
):
    ...
    data = {"customer_charge": CUSTOMER_CHARGE, "rates": rates}
...
if __name__ == "__main__":
    pdf_path = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "/home/sdague/Downloads/Gas & Electric Supply Prices.pdf"
    )
